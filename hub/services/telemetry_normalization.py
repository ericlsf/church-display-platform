...
def _collection_count(value):
    ...
    if isinstance(value, dict):
        for key in ("items", "files", "media", "entries", "playlist"):
            child = value.get(key)
...
def normalize_media_count(health):
    ...
    count = _to_int(numeric)
    if count is not None:
        return max(0, count)

    collection = _first(health, [
        ("playlist",), ("playlist_items",), ("media_files",),
        ("local_media",), ("media","files"), ("media","items"),
        ("player","playlist"), ("playback","playlist"),
        ("manifest",), ("manifest","items"),
        ("content","files"), ("content","items"),
    ])
