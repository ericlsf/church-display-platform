...
class FleetOverviewTests(unittest.TestCase):
    ...
    def test_displays_renders_fleet_cards_and_controls(self, build_state, load_config):
        ...
        self.assertIn("<h2>Displays</h2>", body)
        ...
        self.assertIn("Take screenshot", body)
        self.assertIn("View logs", body)
        self.assertIn("Latest activity", body)
        self.assertIn("Notifications", body)
        self.assertLess(body.index("fleet-notification-panel"), body.index("fleet-card-grid"))
        self.assertLess(body.index("fleet-card-grid"), body.index("Latest activity"))
